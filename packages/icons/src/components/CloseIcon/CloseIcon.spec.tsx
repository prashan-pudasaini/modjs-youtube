...
describe('<CloseIcon>', () => {
    context('Render', () => {
        it('should render default CloseIcon component', () => {
            cy.mount(
                <ModThemeProvider theme={ModLightTheme}>
                    <CloseIcon />
                </ModThemeProvider>,
            )
            cy.get('[data-test="close-icon"]').should('exist')
        })
    })
    context('Props', () => {
        it('should include only valid props in the DOM element and not include any custom attributes from modjs', () => {
            ...
            cy.get('[data-test="close-icon"]').should('have.attr', 'class')
            cy.get('[data-test="close-icon"]').should('have.attr', 'style')
            cy.get('[data-test="close-icon"]').should(
                'have.attr',
                'data-testid',
            )
            cy.get('[data-test="close-icon"]').should('not.have.attr', 'm')
            cy.get('[data-test="close-icon"]').should('not.have.attr', 'p')
        })
    })
    context('Global and Custom Styles', () => {
        ...
        it('should add inline styles with highest precedence', () => {
            cy.mount(
                <ModThemeProvider theme={ModLightTheme}>
                    <CloseIcon style={{ margin: '32px', padding: '32px' }} />
                </ModThemeProvider>,
            )
            cy.get('[data-test="close-icon"]')
                .should('exist')
    ...
    context('Interactions and Behavior', () => {
        ...
        it('should apply fill property from current theme', () => {
            cy.mount(
                <ModThemeProvider theme={ModLightTheme}>
                    <CloseIcon fill={ModLightTheme.color.success} />
                </ModThemeProvider>,
            )
            cy.get('[data-test="close-icon"]')
                .should('exist')
        ...
        it('should apply height property from current theme', () => {
            cy.mount(
                <ModThemeProvider theme={ModLightTheme}>
                    <CloseIcon height={ModLightTheme.icons.height.lg} />
                </ModThemeProvider>,
            )
            cy.get('[data-test="close-icon"]')
                .should('exist')
        ...
        it('should apply width property from current theme', () => {
            cy.mount(
                <ModThemeProvider theme={ModLightTheme}>
                    <CloseIcon width={ModLightTheme.icons.height.lg} />
                </ModThemeProvider>,
            )
            cy.get('[data-test="close-icon"]')
                .should('exist')
