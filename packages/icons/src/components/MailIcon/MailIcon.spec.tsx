...
describe('<MailIcon>', () => {
    context('Render', () => {
        it('should render default MailIcon component', () => {
            cy.mount(
                <ModThemeProvider theme={ModLightTheme}>
                    <MailIcon />
                </ModThemeProvider>,
            )
            cy.get('[data-test="mail-icon"]').should('exist')
        })
    })
    context('Props', () => {
        it('should include only valid props in the DOM element and not include any custom attributes from modjs', () => {
            ...
            cy.get('[data-test="mail-icon"]').should('have.attr', 'class')
            cy.get('[data-test="mail-icon"]').should('have.attr', 'style')
            cy.get('[data-test="mail-icon"]').should('have.attr', 'data-testid')
            cy.get('[data-test="mail-icon"]').should('not.have.attr', 'm')
            cy.get('[data-test="mail-icon"]').should('not.have.attr', 'p')
        })
    })
    context('Global and Custom Styles', () => {
        ...
        it('should add inline styles with highest precedence', () => {
            cy.mount(
                <ModThemeProvider theme={ModLightTheme}>
                    <MailIcon style={{ margin: '32px', padding: '32px' }} />
                </ModThemeProvider>,
            )
            cy.get('[data-test="mail-icon"]')
                .should('exist')
    ...
    context('Interactions and Behavior', () => {
        ...
        it('should apply fill property from current theme', () => {
            cy.mount(
                <ModThemeProvider theme={ModLightTheme}>
                    <MailIcon fill={ModLightTheme.color.success} />
                </ModThemeProvider>,
            )
            cy.get('[data-test="mail-icon"]')
                .should('exist')
        ...
        it('should apply height property from current theme', () => {
            cy.mount(
                <ModThemeProvider theme={ModLightTheme}>
                    <MailIcon height={ModLightTheme.icons.height.lg} />
                </ModThemeProvider>,
            )
            cy.get('[data-test="mail-icon"]')
                .should('exist')
        ...
        it('should apply width property from current theme', () => {
            cy.mount(
                <ModThemeProvider theme={ModLightTheme}>
                    <MailIcon width={ModLightTheme.icons.height.lg} />
                </ModThemeProvider>,
            )
            cy.get('[data-test="mail-icon"]')
                .should('exist')
